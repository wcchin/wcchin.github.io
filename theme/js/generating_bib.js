...
function get_bibtex_modal(bibitem) {
  var line = '';
  //line += '<button class="button is-info is-small is-rounded" onclick="openModal(event, \'modal-'+bibitem["citationKey"]+'\')">bibtex</button>';
...
function get_description(bibitem) {
  var line = '';
  //line += '<div class="tags has-addons">';
  //line += '  <span class="tag is-dark is-small is-rounded is-outlined" onclick="openModal(event, \'desc-' + bibitem["citationKey"] + '\')">Abstract</span>';
  //line += '</div>';
  line += '<div id="desc-'+bibitem["citationKey"]+'" class="modal">';
  line += '  <div class="modal-background"></div>';
  line += '  <div class="modal-card">'
  line += '    <header class="modal-card-head"><p class="title is-4"><b>'+bibitem["entryTags"]["title"]+'</b></p>';
  line += '    </header>';
  line += '    <section class="modal-card-body">';
  
  if (bibitem["entryTags"].hasOwnProperty('author')) {
    let names = get_authors(bibitem["entryTags"]["author"]);
    line += '      <p>' + names; 
    if (bibitem["entryTags"].hasOwnProperty('year')) {
      line += '      (' + bibitem["entryTags"]["year"] + ')'; 
    }
    line += '      </p>';
  }
  if (bibitem["entryTags"].hasOwnProperty('journal')) {
    line += '      <p align="right"><i>' + bibitem["entryTags"]["journal"] + '</i></p>';
  }
  if (bibitem["entryTags"].hasOwnProperty('booktitle')) {
    line += '      <p align="right"><i>' + bibitem["entryTags"]["booktitle"] + '</i></p>';
  }
  if (bibitem["entryTags"].hasOwnProperty('doi')) {
    let doi = bibitem["entryTags"]["doi"].replace(/[\\\{|\}]/g, '');
    //let doitxt = '<p align="right"><a href="https://doi.org/' + doi + '" target="_blank"><img src="https://zenodo.org/badge/DOI/' + doi + '.svg"></a></p>';
    let doitxt = '<div style="display: flex; justify-content: flex-end"><div class="tags has-addons">';
    doitxt += '<span class="tag is-dark is-small">DOI</span>';
    doitxt += '<a class="tag button is-info is-small" href="https://doi.org/' + doi + '">' + doi + '</a>';
    doitxt += '</div></div>';
    line += '    '+ doitxt;
  }
  if (bibitem["entryTags"].hasOwnProperty('abstract')) {
    line += '      <p align="center" class="title is-5"><b>Abstract</b></p>';
    line += '      <p align="justify">' + bibitem["entryTags"]["abstract"] + '</p>'; 
  }
  if (bibitem["entryTags"].hasOwnProperty('keywords')) {
    line += get_keywords_line(bibitem["entryTags"]);
  }
  line += '    </section>';
  line += '    <button class="modal-close is-large" aria-label="close"  onclick="closeModal(event, \'desc-'+bibitem["citationKey"]+'\')"></button>';
  line += '    <footer class="card-footer">';
  line += '      <a class="button is-link card-footer-item" href="/publications/'+bibitem["entryTags"]["note"]+'.html" id="readmore">Read more...</a>';
  line += '      <button class="button is-dark card-footer-item" onclick="closeModal(event, \'desc-'+bibitem["citationKey"]+'\')">Close</button>';
  line += '    </footer>';
  line += '  </div>'
  line += '</div>';
  return line
...
function get_bib_buttons(bibitem) {
  var line = '';
  line += '<div class="tags has-addons">';
  line += '  <span class="tag button is-dark is-small is-rounded is-outlined" onclick="openModal(event, \'modal-'+bibitem["citationKey"]+'\')">bibtex</span>';
  line += '  <span class="tag button is-dark is-small is-rounded is-outlined" onclick="openModal(event, \'desc-' + bibitem["citationKey"] + '\')">Abstract</span>';
  line += '  <a class="tag button is-dark is-small is-rounded is-outlined" href="/publications/'+bibitem["entryTags"]["note"]+'.html">Read more...</a>';
  line += '</div>';
  line += get_bibtex_modal(bibitem);
  line += get_description(bibitem);
  return line
}
      ...
      do {
        ...
        if (str) {
          if (typeof(latex_to_unicode[key]) != "undefined") {
            ...
            //console.log(value, newkey, 3);
            if (typeof(latex_to_unicode[newkey]) != "undefined") {
              value = value.replace(key, latex_to_unicode[newkey]);
            } else {
              //console.log(4);
              value = value.replace(key, ""); // exhausted solution, remove everything weird
              str = "";
            }
          }
        } else {
...
function get_authors(names) {
  var all_names = names.split(" and ");
  //console.log(all_names);
  var names = "";
  for (i=0; i<all_names.length; i++) {
    var this_name = all_names[i].split(",");
    //console.log(this_name);
    if (i>0 && i<all_names.length-1) {
      names += ", ";
    } else if (i==all_names.length-1 && i!=0) {
      names += " and ";
    }
    var sur_name = fixValue(this_name[0]).trim();
    var given_name = fixValue(this_name[1]).trim();
    var given_name2 = "";
    if (given_name.match(/[\u3400-\u9FBF]/)) {
      given_name2 = given_name;
    } else {
      if (given_name.includes(" ")) {
        var temp = given_name.split(" ");
        for (j=0; j<temp.length; j++) {
          given_name2 += temp[j][0] + ". ";
        }
        given_name2 = given_name2.trim();
      } else if (given_name.includes("-")) {
        var temp = given_name.split("-");
        for (j=0; j<temp.length; j++) {
          given_name2 += temp[j][0] + ".-";
        }
        if (given_name2.slice(-1)=="-") {
          given_name2 = given_name2.slice(0, -1);
        }
      } else {
        given_name2 = given_name[0] + ".";
      }
    }
    //console.log(sur_name);
    //console.log(given_name);
    if (given_name.match(/[\u3400-\u9FBF]/)) {
      var this_name2 = sur_name + given_name2;
    } else {
      var this_name2 = sur_name + ", " + given_name2;
    }
    names += this_name2;
...
function process_article(bibitem) {
  ...
  if (item.hasOwnProperty('volume') && item.hasOwnProperty('number')) {
    line += ", " + item["volume"] + "(" + item["number"] + ")";
...
function load_bibtex(fp) {
  ...
  //console.log(yr);
  var bibcontent = document.getElementById('timeline-box');
  var xhr = new XMLHttpRequest();
  var text_box = '';
  xhr.onreadystatechange = function() {
    if (xhr.readyState == 4 && xhr.status == 200) {
      //bibcontent.innerHTML += '<header class="timeline-header"><span class="tag is-medium is-primary">Most Recent</span></header>'
      ...
      for (let i=0;i<years.length;i++) {
        ...
        yritem = '<header class="timeline-header"><span class="tag is-dark is-large">'+yr+'</span></header>';
        //yritem += '<div class="timeline-item" id="bibtex-'+yr+'"></div>';
        bibcontent.innerHTML += yritem;
        //bibcontent.innerHTML += '<div class="block" id="bibtex-'+yr+'"><h3 class="milestone">'+yr+'</h3></div>';
      //}
      //for (var yr in year_docs) {
        ...
        if (journal_article.length>0) {
          //text_response += '<b> Journal Article </b>';
          text_response += '<div class="timeline-item">';
          text_response += '<div class="timeline-marker is-icon"><i class="fa fa-pen-nib"></i></div>'
          text_response += '<div class="timeline-content"><p class="heading">Journal Article</p>'
          for (let i=0; i<journal_article.length;i++){
            let item = journal_article[i];
            var txt = process_article(item);
            text_response += txt;
          }
          text_response += '</div></div>';
        }
        if (incollection.length>0) {
          //text_response += '<b"> Book chapter </b>';
          text_response += '<div class="timeline-item">';
          text_response += '<div class="timeline-marker is-icon"><i class="fa fa-book-open"></i></div>'
          text_response += '<div class="timeline-content"><p class="heading">Book chapter</p>'
        ...
        if (proceedings.length>0) {
          //text_response += '<b> Conference paper </b>';
          text_response += '<div class="timeline-item">';
          text_response += '<div class="timeline-marker is-icon"><i class="fa fa-comments"></i></div>'
          text_response += '<div class="timeline-content"><p class="heading">Conference paper</p>'
        ...
        if (preprint_article.length>0) {
          //text_response += '<b> Preprint Article </b>';
          text_response += '<div class="timeline-item">';
          text_response += '<div class="timeline-marker is-icon"><i class="fa fa-paper-plane"></i></div>'
          text_response += '<div class="timeline-content"><p class="heading">Preprint Article</p>'
        ...
        if (thesis.length>0) {
          //text_response += '<b> Dissertation/Thesis </b>';
          text_response += '<div class="timeline-item">';
          text_response += '<div class="timeline-marker is-icon"><i class="fa fa-graduation-cap"></i></div>'
          text_response += '<div class="timeline-content"><p class="heading">Dissertation/Thesis</p>'
        ...
        //text_response += "<hr>";
        //document.getElementById('bibtex-'+yr).innerHTML += text_response;
        bibcontent.innerHTML += text_response;
      }
      //bibcontent.innerHTML += '<div class="timeline-header"><span class="tag is-medium is-primary">Earliest</span></div>'
    }
  }
  xhr.open('GET', fp);
  xhr.send();
...
function loading_bibs(fp) {
  //console.log(fp)
  //let fp = './theme/my_publications.bib';
